...
"""
Encontre el problema, el problema, es que cuando tengo la camara abierta
        if variable.VIEWCAMARA[0] == 1:
            cv2.namedWindow('frame', cv2.WINDOW_NORMAL)
            cv2.resizeWindow('frame', 450,450)
            cv2.imshow("frame", frame)

E intento correr cualquier cosa que tenga show_notification, me sale este error:

The program 'python3' received an X Window System error.
This probably reflects a bug in the program.
The error was 'BadWindow (invalid Window parameter)'.
  (Details: serial 12431 error_code 3 request_code 15 minor_code 0)
